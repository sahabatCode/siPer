...
<?php
  ...
  if ($_SESSION['status']=='admin'){
    ...
    $tampil = mysql_query("SELECT * FROM buku WHERE id_buk='$_GET[id]'");
    $y      = mysql_fetch_array($tampil);
    ?>
    <div class="judul"><h2> Ubah Buku</h2></div>
       <div class="area_main">
         <form method="POST" action="<?php echo "$aksi?mod=buk&aksi=ubh_buk"; ?>">
        <table class="form">
		  <tr>
            <td>ID Barang</td>
            <td>:</td>
            <td>
              <input type="text" name="id_buk" id="id_buk" maxlength="6" value="<?php echo $y['id_buk']; ?>" size="10" disabled="disabled" />
              <input type="hidden" name="id_buk_h" id="id_buk_h" value="<?php echo $y['id_buk']; ?>" size="10" />            
            </td>
          </tr>
		  <tr>
            <td>Nama Barang</td>
            <td>:</td>
            <td><input type="text" name="nama_buk" id="nama_buk" value="<?php echo $y['nama_buk']; ?>" size="30" /></td>
          </tr>
		  <tr>
            <td>Merk</td>
            <td>:</td>
		  ...
		  <tr>
            ...
            <td><input type="text" name="stock" id="stock" value="<?php echo $y['stock']; ?>" size="5px" /></td>
             ...
             <tr>
               <td></td>
               <td></td>
               <td><input type="submit" class="button" value="Ubah"> <input type="button" class="button" value="Batal" onclick="buk_bat()"></td>
             </tr>
           </table>
		 </form>
       </div>
